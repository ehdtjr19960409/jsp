...
public class TestServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//getWriter() 메소드는 화면에 출력을 하기 위해 사용하며 출력 스틀림을 반환(출력하는 객체)
		//	이 객체를 활용하게 되면 사용자의 화면에 글씨를 뿌려줄 수 있다
		//append() 메소드는 문자열을 연결해주는 메소드
		//request.getContextPath() : 리퀘스트 객체로부터 ContextPath라는 것을 얻어오는데 컨텍스트패스는 현재 우리의 사이트, 우리의 웹어플리케이션의 루트 경로를 의미한다
...
//		out.print("<html>"); //out.print() : PrintWriter 객체를 사용하여 HTML코드를 생성하고 부라우저로 전송
//		out.print("<head>");
...
//		out.close();
		...
		//getParameter(String agr) 꺼내고 싶으 데이터의 이름을 넘겨주면된다
		...
		//이 객체에서 print()메소드를 사용하면 클라이언트 화면에 보여줄 문자열을 응답객체에 담을 수 있다
		
		
//		response.setContentType("text/html;charset=UTF-8");
//		//ContentType은 현재 페이지의 컨텐츠들을 의미한다, 다른 의미로는 현재 페이지에 뿌려지는 데이터들의 정보를 담고있다
...
//		out.close();
		...
		//위의 코드도 여전히 서블릿 형식으로 사용하고 있으므로 결과 페이지를 만들 result.jsp 파일을 만들고 jsp파일로 실행시킬 것이다
		
		//forward 방식 사용
		//getRequestDispatcher("경로").forward(request, response)
		//다른 서블릿이나 jsp로 요청을 전달한다
		//주로 클라이언트의 요청을 받아 다른 페이지로 이동을 시킬 때 사용한다
		//getRequestDispatcher는 request 객체가 가지고 있으며, request 객체를 전달할 파일 경로를 매개변수로 넘겨줘야한다

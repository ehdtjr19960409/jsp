...
public class JoinOkController implements Execute{
	...
	public Result execute(HttpServletRequest request, HttpServletResponse response)
		...
		result.setRedirect(true); //++++ Result 객체 추가 후 작성 //어떻게 보낼지
		result.setPath(request.getContextPath()); //++++ Result 객체 추가 후 작성 //어디로 보낼지
		return result; //++++ 반환값 수정 후 프론트컨트롤러로 이동
	}
	
	
	// 프론트컨트롤러에서 작성한 코드를 처리하는데,
...
//		
//		
//		MemberDAO memberDAO = new MemberDAO(); // +++ DAO클래스의 메소드 실행하기 위한 객체 생성
//		MemberDTO memberDTO = new MemberDTO();
//		System.out.println(memberDTO);
//		memberDTO.setMemberId(request.getParameter("memberId"));
//		memberDTO.setMemberPassword(request.getParameter("memberPassword"));
